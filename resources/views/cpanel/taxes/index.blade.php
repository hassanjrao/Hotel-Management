    ...
    <div class="content">
        ...
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                ...
                <a href="{{ route('cpanel.taxes.create') }}" class="btn btn-primary push">Add</a>
                        ...
                        <tbody>
                            @foreach ($taxes as $ind => $tax)
                                <tr>
                                    ...
                                    <td>{{ $tax->amount }}</td>
                                    <td>
                                        <x-release-status-badge :code="$tax->releaseStatus->code" :label="$tax->releaseStatus->name" />
                                    ...
                                    <td>
                                        ...
                                        <div class="btn-group me-2 mb-2" role="group"
                                            ...
                                            <a href="{{ route('cpanel.taxes.release', ['release_status' => 'published', 'tax_id' => $tax->id]) }}"
                                                class="btn btn-xs btn-outline-success " data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Publish">
                                                <i class="fa fa-check"></i>
                                            </a>
                                            <a href="{{ route('cpanel.taxes.release', ['release_status' => 'not_published', 'tax_id' => $tax->id]) }}"
                                            ...
                                            <a href={{ route('cpanel.taxes.edit', ['tax' => $tax]) }}
                                            ...
                                            <form id="form-{{ $tax->id }}"
                                                action="{{ route('cpanel.taxes.destroy', $tax->id) }}"
                                                method="POST">
                                                @method('DELETE')
                                                @csrf

                                                <a onclick="confirmDelete({{ $tax->id }})"
